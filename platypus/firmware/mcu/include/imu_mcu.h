...
// initializes the IMU
void setupIMU(int samplerate_mpu, uint8_t m_mpu_address, _Bool use_mpu_fifo, _Bool m_start_env, _Bool m_start_com);
void stopIMU();
...
// raw 16Bit values, no conversion, send it to the Atom
...
uint32_t compP(int32_t adc_P, int32_t adc_T);//[Pa]
uint32_t compH(int32_t adc_H, int32_t adc_T);

// reads the IMU ES data, compensates, and returns values via reference
...
struct BME_calibration m_env_calib;
_Bool m_init_env;

int m_HCalib_X, m_HCalib_Y, m_HCalib_Z; //float
