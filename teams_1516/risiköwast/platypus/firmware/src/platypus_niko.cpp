...
enum msg_type{server_msg, client_msg};
enum command{update, response, animation, display_text};
...
void platypus::parseIncommingMessage(char *buffer) {
  ...
  switch(message) {
	  ...
	  case client_msg:
		...
		// Extract command
		ptr = strtok(NULL, delimiter);
		command client_command = atoi(ptr);
		...
		switch(client_command) {
			...
			case response:
				...
				break;
			case animation:
				// Extraxt data (start/stop animation)
				ptr = strtok(NULL, delimiter);
				action client_action = atoi(ptr);
				case(client_action) {
					case start:
			...
			default:
				...
				printf("Bullshit client_command\n");
		}	
		break;
